...
const normalizeOrder = (order, index) => {
  ...
  return {
    ...
    currentSituation: first(order.currentSituation, order.situation, order.current, order.context, order.marketContext),
    currentSituationText: localized(order, 'currentSituation', order.situation, order.current, order.context, order.marketContext),
    suggestion: first(order.suggestion, order.advice, order.actionPlan, order.recommendationText),
    suggestionText: localized(order, 'suggestion', order.advice, order.actionPlan, order.recommendationText),
