...
export default function Portfolio() {
  ...
  return <section><div className="page-head"><div><p className="eyebrow">{t('portfolioEyebrow')}</p><h1>{t('portfolioTitle')}</h1><p>{t('portfolioSubtitle')}</p></div></div>
    ...
    <div className="panel trading-stats"><h2>{t('tradingStatistics')}</h2><div className="stats-grid">
      ...
      <span>{t('bestRealizedTrade')}<strong>{tradingStats.bestRealizedTrade ? `${tradingStats.bestRealizedTrade.symbol} ${money(tradingStats.bestRealizedTrade.realizedPL, tradingStats.bestRealizedTrade.currency || singleCurrency || 'USD')}` : 'N/A'}</strong></span>
      <span>{t('worstRealizedTrade')}<strong>{tradingStats.worstRealizedTrade ? `${tradingStats.worstRealizedTrade.symbol} ${money(tradingStats.worstRealizedTrade.realizedPL, tradingStats.worstRealizedTrade.currency || singleCurrency || 'USD')}` : 'N/A'}</strong></span>
      ...
      <span>{t('largestPosition')}<strong>{tradingStats.largestPosition ? `${tradingStats.largestPosition.symbol} ${money(tradingStats.largestPosition.marketValue, tradingStats.largestPosition.quote.currency)}` : 'N/A'}</strong></span>
