...
export default function TradeModal({ side, symbol, defaultPrice, onClose, onSave }) {
  const [form, setForm] = useState({ price: defaultPrice, shares: '', ...localDateTime(), note: '' })
  const update = (event) => setForm({ ...form, [event.target.name]: event.target.value })
  const submit = (event) => {
    event.preventDefault()
    const { time, ...trade } = form
    onSave({ ...trade, side, symbol, price: Number(form.price), shares: Number(form.shares), date: new Date(`${form.date}T${time}`).toISOString() })
  }
  return (
