...
export default function AIAnalysis() {
  ...
  return <section><div className="page-head"><div><p className="eyebrow">{t('aiEyebrow')}</p><h1>{t('aiTitle')}</h1><p>{t('aiSubtitle')}</p></div></div>
    ...
    <details className="panel ai-context-panel advanced-context-panel">
      <summary><div><h2>{t('legacyContextTitle')}</h2><p>{t('legacyContextSubtitle')}</p></div><span>+</span></summary>
      <div className="advanced-context-body">
        ...
        <div className="ai-preview-panel"><h2>{t('aiPromptPreview')}</h2>{preview ? <pre>{preview}</pre> : <div className="empty-inline">{t('aiPromptEmpty')}</div>}</div>
      </div>
    </details>
  </section>
}
