...
const normalizeOrderCommitment = (order) => migrateOrder({
  ...
  currentSituation: order.currentSituation || order.currentSituationText?.en || order.currentSituationText?.zh || '',
  currentSituationText: order.currentSituationText,
  suggestion: order.suggestion || order.suggestionText?.en || order.suggestionText?.zh || '',
  suggestionText: order.suggestionText,
  plannedOrder: order.plannedOrder || order.plannedOrderText?.en || order.plannedOrderText?.zh || '',
  plannedOrderText: order.plannedOrderText,
  note: order.note || order.noteText?.en || order.noteText?.zh || '',
  noteText: order.noteText,
  reasonText: order.reasonText,
  riskText: order.riskText,
  reEntryPlanText: order.reEntryPlanText,
  legs: (order.legs || []).map((leg) => ({
    id: leg.id,
    side: leg.side,
    label: leg.label,
    labelText: leg.labelText,
    price: Number(leg.price || 0),
    shares: Number(leg.shares || 0),
    amount: Number(leg.amount || 0),
    conditionText: leg.conditionText,
    noteText: leg.noteText,
  })),
